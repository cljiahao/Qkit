...
- `coffee-cart.sql` — seeds one active booth ("Kopitiam Cart") under the
  ...
  panel and payment-queue e2e specs have something to render.
...
- `demo-two-booths.sql` — a richer local demo dataset: upserts the "Test"
  ...
  re-inserts two active booths — "Kopitiam Cart" (PayNow-enabled) and an
  "Ice Cream Cart" (queue-only, no payment method) — for demo-video recording
  and manual multi-booth testing. Explicitly never run via `db reset`.
