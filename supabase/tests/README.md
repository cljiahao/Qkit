...
- `rls.test.sql` — a single pgTAP file (`plan(115)`, run inside one rolled-back
  ...
  - Payment-first order numbering (migration `0087`): a payment-required
    ...
    retried call for the same order; `anon` cannot read the `payment-proofs`
    storage bucket (a real row is seeded first, as the privileged role, so
    the check proves RLS actually filters it out).
