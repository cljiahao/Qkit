-- A booth with no printer connected has no physical ticket to track what's
-- being worked on, so a QR order should land as 'pending' (incoming, needs a
-- vendor "Start now" tap) instead of auto-starting into 'preparing' — same
-- as today's requires_arrival_confirm gate, just a second independent reason
-- an order can start pending. place_walkup_order is untouched: a vendor who
-- typed the order in already knows about it, no accept step needed.
