...
CREATE OR REPLACE FUNCTION public.vendor_entitled(p_vendor uuid)
RETURNS boolean
LANGUAGE sql
SECURITY DEFINER
STABLE
...
REVOKE ALL ON FUNCTION public.vendor_entitled(uuid) FROM PUBLIC;
...
AS $$
  SELECT
    public.vendor_entitled(p_vendor)
...
AS $$
  ...
  SELECT
    b.is_active
    AND (
      public.vendor_entitled(b.vendor_id)
      OR b.id = (
...
REVOKE EXECUTE ON FUNCTION public.apply_order_stock_delta(uuid, jsonb, int) FROM PUBLIC;
