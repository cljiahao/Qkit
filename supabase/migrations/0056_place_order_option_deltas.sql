-- Extend place_order's options-validation loop to also sum each selected
-- choice's price_delta_cents/cost_delta_cents into the line's total/cost —
-- same trust boundary as the base item price: this remains the only place
-- a price is authoritative, everything client-side stays informational.
-- See docs/superpowers/specs/2026-07-18-menu-choice-price-delta-design.md.
-- Recreate verbatim from 0055 with this one addition.
  ...
  FOR line IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    ...
    IF line ? 'options' AND jsonb_typeof(line->'options') = 'array' THEN
      ...
      FOR opt IN SELECT * FROM jsonb_array_elements(line->'options') LOOP
        -- Replaces the old EXISTS-only check: SELECT INTO leaves v_delta_price/
        -- v_delta_cost NULL and FOUND false when no {group,choice} matches,
        -- same "unknown option" failure mode as before.
