...
CREATE OR REPLACE FUNCTION qkit.orders_stock_sync()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = qkit
...
BEGIN
  IF (TG_OP = 'INSERT') THEN
    IF NEW.status <> 'cancelled' THEN
      PERFORM qkit.apply_order_stock_delta(NEW.booth_id, NEW.items, 1);
    END IF;
...
CREATE TRIGGER orders_stock_sync_ins
  AFTER INSERT ON qkit.orders
  FOR EACH ROW EXECUTE FUNCTION qkit.orders_stock_sync();
...
AS $$
  ...
  FROM caps
  LEFT JOIN qkit.booth_item_sold s
