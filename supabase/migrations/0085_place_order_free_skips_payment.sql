-- v_expects_payment ignored total_cents, so a $0 order still showed a pay
-- panel whenever the booth had a payment method configured. Now requires
-- v_total > 0 too, in both place_order and place_walkup_order.
...
-- Backfill: no real vendors onboarded yet, safe to correct existing rows.
UPDATE qkit.orders
SET payment_status = 'not_required', payment_method_kind = NULL
WHERE total_cents = 0 AND payment_status = 'pending';
