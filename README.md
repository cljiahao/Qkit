...
- `package.json` — scripts (`dev`/`build`/`test`/`test:mutation`/`test:e2e`/`check`/`format`/`demo:record`/`demo:compose`/`prepare`), the dependency set (`@merqo/ui` — the shared Merqo-family component package, installed from a GitHub release tag (`github:cljiahao/merqo-ui#v0.10.0`) rather than npm; Next 16, `@supabase/ssr` + `@supabase/supabase-js`, Radix/shadcn deps, `react-hook-form` + `zod`, `recharts`, `react-qr-code`, `@icons-pack/react-simple-icons` (real brand-logo icons), the dev toolchain (`vitest`, `@playwright/test`, `@stryker-mutator/*`, `eslint` + `eslint-plugin-sonarjs`, `husky`, `prettier`). `prepare` runs `husky`.
