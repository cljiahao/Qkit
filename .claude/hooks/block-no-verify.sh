...
if echo "$scan" | grep -qE '\bgit\b' && echo "$scan" | grep -qE '(^|[[:space:]])HUSKY=0|(^|[[:space:]])HUSKY_SKIP_HOOKS=|core\.hooksPath[[:space:]]*='; then
  echo "BLOCKED: HUSKY=0 / HUSKY_SKIP_HOOKS / 'git -c core.hooksPath=...' disables husky's hook layer (pre-commit/commit-msg/pre-push) — the same bypass as --no-verify. Fix the failure instead." >&2
  exit 2
fi
