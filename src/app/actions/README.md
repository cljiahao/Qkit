...
- `events.ts` — `logEvent(type: EventType, metadata?)`. `EventType` is a Zod enum allowlist (`landing_cta`, `upgrade_cta`, `booth_view`, `order_placed`); rejects any other type and drops non-object or >1000-char-serialized `metadata` rather than risk a bad insert. Best-effort/fire-and-forget: inserts into the `events` table via the normal (RLS-scoped) client and never throws.
- `events.test.ts` — Vitest coverage for `logEvent`: allowlisted insert, rejection of an out-of-allowlist type, dropping oversized/non-object metadata, and never throwing when the insert itself fails.
