...
- `purchase.ts` — `requestUpgrade(option: "event" | "monthly"): Promise<ActionResult>`. Validates via a Zod enum, requires a signed-in vendor, rate-limits per vendor (`upgrade-request:<vendorId>`, 5/60s — the idempotency check below only blocks a repeat of the _same_ kind while pending, not an authenticated vendor scripting alternating calls to flood the admin queue), and is idempotent: if a pending `purchase_requests` row of the same `kind` already exists it returns success as a no-op instead of inserting a duplicate.
- `purchase.test.ts` — tests the Zod rejection, the sign-in requirement, the rate-limit denial (before the idempotency check even runs), the idempotent no-op, a successful insert, and insert-failure handling.
