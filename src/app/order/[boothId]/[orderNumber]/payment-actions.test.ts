...
//   from("orders").update().eq().eq().eq().neq() → { error }
// `neq` is the terminal and is set per-test to a configurable { error }.
// `update` is a spy so we can assert the payload and that it ran at all.
...
describe("claimPayment", () => {
  it("claims a valid pending order (update runs, returns success)", async () => {
    const res = await claimPayment(BOOTH, ORDER);
  ...
  it("rejects an invalid booth id before creating the client", async () => {
    const res = await claimPayment("not-a-uuid", ORDER);
  ...
  it("rejects an empty order number before creating the client", async () => {
    const res = await claimPayment(BOOTH, "");
