...
- `actions.ts` — `createVendor(input)` server action: validates `input`
  against `vendorSchema`, requires a signed-in user, then inserts
  `{ id: user.id, name }` into `vendors`. Treats Postgres unique-violation
  (`23505`, i.e. the row already exists) as success rather than an error, so
  a double-submit or back-nav retry is harmless.
