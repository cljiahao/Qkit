...
- `pass-countdown.tsx` — `PassCountdown({ expiresAt })` client component: live minute-ticking countdown (`useNow(60_000)`; a non-breaking space holds the line until the client clock is set, so the SSR'd text can't mismatch) formatted as `Xd Yh left` / `Xh Ym left` / `Xm left`, with the expiry rendered in a fixed SGT string (`sgtWeekdayTime`) to avoid a server/client hydration mismatch from a runtime-timezone `toLocaleString`.
