...
The menu-manager CSV covers `name,description,price,available` only —
`option_groups` (Size/Spice/Add-ons style customization) is UI-only,
...
Cost delta and allergens stay UI-only — same reasoning as the item-level
CSV already leaving `stock`/`cost_cents`/`image_url`/`allergens` out: those
live behind "Advanced" in the UI specifically because most vendors don't
...
surface trades accuracy for convenience the wrong way. A vendor who needs
per-choice cost/allergens sets them in the guided UI after import, same as
today.
...
plain 4-column export/template still parses unchanged, new columns just
...
```csv
name,description,price,available,group_name,group_type,choice_label,choice_price
Kopi,Local coffee,1.40,true,,,,
,,,,Style,one,O (black),
,,,,Style,one,C (evaporated milk),0.20
,,,,Style,one,Normal (condensed milk),0.20
Teh,Local tea,1.40,true,,,,
```

- A row with `name` filled is an **item row** — parsed exactly as today
  (`name,description,price,available`), plus the trailing columns ignored
  for this row.
...
- An item with **no choice rows** in the imported file keeps its existing
  ...
  behavior of never touching fields the CSV doesn't carry (`image_url`,
  `cost_cents`, `stock` are preserved today the same way).
...
- Cost delta and allergens (see Scope decision above).
...
- Any change to the item-level CSV's existing columns or behavior.
